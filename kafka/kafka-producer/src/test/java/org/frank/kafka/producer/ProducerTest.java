...
//        props.put("bootstrap.servers", "47.242.251.45:9092");
        props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "47.242.251.45:9092");
...
//        props.put("acks", "all");
    ...
    public void sendMessageTest(){
        ...
        for(int i=0;i<3 ;i++){
            Future<RecordMetadata> future = producer.send(new ProducerRecord<>("kafka-sp-topic-1","frank-key"+i, "frank-value"+i));
            try {
                //不关心结果则不用写这些内容
    ...
    @Test
    public void sendMessageWithSelfDefinedPartitionTest(){
        Properties properties = getProperties();
        properties.put("partitioner.class", "org.frank.kafka.producer.config.MyPartitioner");
        Producer<String,String> producer = new KafkaProducer<>(properties);
        for(int i=0;i<3 ;i++){            
            producer.send(new ProducerRecord<>("kafka-sp-topic-1", "frank-key", "frank-value" + i), new Callback() {
                @Override
                public void onCompletion(RecordMetadata metadata, Exception exception) {
                    if(exception == null){
                        System.out.println("发送状态："+metadata.toString());
